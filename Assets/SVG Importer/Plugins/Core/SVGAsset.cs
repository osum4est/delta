    ...
    public class SVGAsset : ScriptableObject
    {
        ...
        public Mesh mesh
        {
            get {
                ...
                return CloneMesh(sharedMeshReference);;
            }
        }
...
//                        Texture2D atlasTextureReference = null;
                        if(SVGAtlas.Instance.atlasTextures != null && SVGAtlas.Instance.atlasTextures.Count > 0)
                        {
//                            atlasTextureReference = SVGAtlas.Instance.atlasTextures[0];
...
//                            Material material;
        ...
        public Texture2D[] atlasTextures
        {
            get {
				...
				if(UnityEditor.EditorApplication.isPlaying)
                {
					...
					{
						Texture2D atlasTexture = SVGAtlas.GenerateGradientAtlasTexture(_sharedGradients, 64, 2);
        ...
        protected Mesh CloneMesh(Mesh original)
        {
            if(original == null)
                return null;
        ...
        internal string _editor_UpdateInfo()
        {
            if(_sharedMesh == null)
            {
            ...
            if(_sharedMesh.colors32 != null && _sharedMesh.colors32.Length > 0)
                output += ", Color";
            if(_sharedMesh.uv != null && _sharedMesh.uv.Length > 0)
                output += ", UV";
            if(_sharedMesh.uv2 != null && _sharedMesh.uv2.Length > 0)
                output += ", UV2";
		...
		internal Material[] _editor_sharedMaterials
		{
			get {
				if(_sharedMaterials == null || _sharedMaterials.Length == 0)
				{
					...
					AssignMaterialGradients(_sharedMaterials, atlasTextures[0], SVGAtlas.gradientShapeTexture, 64, 2);
	            }
		...
		internal Material _editor_UIMaterial
		{
			get {
				if(_sharedUIMaterial == null)
				{
					...
					AssignMaterialGradients(_sharedUIMaterial, atlasTextures[0], SVGAtlas.gradientShapeTexture, 64, 2);
				}
		...
		internal Material _editor_UIMaskMaterial
		{
			get {
				if(_sharedUIMaskMaterial == null)
				{
					...
					AssignMaterialGradients(_sharedUIMaskMaterial, atlasTextures[0], SVGAtlas.gradientShapeTexture, 64, 2);
				}
        ...
        internal string _editor_Info
        {
            get {
                ...
                if(_documentAsset.infoString == null)
                    return "";

                return _documentAsset.infoString;
            }
        }
